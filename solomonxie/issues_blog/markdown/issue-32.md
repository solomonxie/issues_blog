...
> 提前声明：Jekyll并不简单，必须要正确的看待它。把它和PHP，JSP和Django等放在一起讨论，这样会减少很多失落感。它的学习曲线几乎相当于Wordpress，思路也比较相似。
...
$ bundle install
```
- 然后`bundle`命令会根据文件夹中的`Gemfile`文件下载安装所有模版所需的依赖环境。
...
- 然后重新运行`jekyll serve`就可以在网站上看到更新了。
