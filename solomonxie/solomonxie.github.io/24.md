...
```
import sys, getopt
opts, args = getopt.getopt(sys.argv[1:], 'h:p',['help', 'proxy'])
    for o,a in opts:
        if   o=='-h' or o=='--help' :  print a 
	elif o=='-p' or o=='--proxy':  print 'enabled proxy' 
```
...
- 当前所在文件夹: `os.getcwd()`
- 当前文件名：`__file__`
- 当前文件路径: `os.path.join( os.getcwd(), __file__ )`
