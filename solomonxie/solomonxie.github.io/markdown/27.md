...
```shell
# 将图片文件写入剪切板(不是系统剪切板）
xclip -selection clipboard PATH-TO-IMAGE.png

# 将剪切板内图片存为本地文件 (不是从系统剪切板里得来）
xclip -selection clipboard -target image/png -out > PATH-TO-IMAGE.png
```
