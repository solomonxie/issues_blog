...
## 安装主题
实际上，我们用静态网站不就是为了简单吗，要不然还不如直接用更强大的Wordpress。
所以有时候经常觉得Jekyll的学习成本太高，没必要。
其实如果为了简单，直接下载模版就OK了，没有那么高的学习成本。
语法之类，大概知道一点点就足够了。

但是，安装主题也不是把人家做好的template直接复制过来就能用了。
每个模版设置的变量设置名、依赖的gem包都不一样，还经常需要在本地安装所有依赖包，安装jekyll插件等。真是不简单。
