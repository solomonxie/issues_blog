...
```
You seem to be running Irssi inside tmux, but the TERM environment variable is set to 'xterm-256color'
```

做法是在`~/.zshrc`或`~/.bash_profile`里面（而不是tmuxrc里），添加这个：
```sh
if [[ -n "$TMUX" ]]; then
    export TERM=screen-256color
elif [[ -n "$SSH_CLIENT" && "$HOST" = "machine_b_hostname" ]]; then
    export TERM=screen-256color
else
