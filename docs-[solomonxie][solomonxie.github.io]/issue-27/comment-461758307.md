...
除了第三方客户端的允许，我们还要设置POP。最好放开全部邮件或者最近30天，比如阿里云邮箱：

![image](https://user-images.githubusercontent.com/14041622/52520591-f5687380-2ca5-11e9-8cd1-92fae63f7cfc.png)
...
基本操作：
- 基本：`q`:Quit, `d`:删除当前邮件, `s`:让动作生效, `m`:创建新邮件, `r`:回复当前邮件, `?`:帮助
- 移动：`j/k` 上下移动邮件，<Space>: 向下翻页
- `<Enter>` 打开光标所在的邮件
