...
然后在主配置或自定义配置的`server`模块里定义判断逻辑：
```cfg
server {
   # 其它配置...
...
这里配置不通过，说这里不能放`if directive`，其实不是语法问题，而是nginx编译的时候没有添加`ngx_http_rewrite_module`模块：
http://nginx.org/en/docs/http/ngx_http_rewrite_module.html#if

所以安装PCRE库的以来，和需要重新编译安装nginx并指定`rewrite`模块来支持这个语法。

```sh
# 安装PCRE依赖，官方下载列表：https://ftp.pcre.org/pub/pcre/
wget https://ftp.pcre.org/pub/pcre/pcre2-10.34.zip
gunzip pcre2-10.34.zip
cd pcre2-10.34
# 编译和安装
./configure
