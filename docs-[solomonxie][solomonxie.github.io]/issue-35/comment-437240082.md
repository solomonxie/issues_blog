...
生成证书分为这几步：
- 生成一个私钥 Private key
- 根据私钥生成一个公钥 Public key
- 根据公钥生成一个CSR证书请求文件 Certificate Request
- 根据CSR生成一个证书 Certificate

生成证书：
```sh
# 1. 生成私钥 Private key
$ openssl genrsa -des3 -passout pass:x -out ~/.ssl/PRIVATE.key 2048

# 2. 利用私钥生成公钥  Public key
$ openssl rsa -passin pass:x -in ~/.ssl/PRIVATE.key -out ~/.ssl/PUBLIC.key

# 3. 根据公钥生成一个CSR证书请求 Request
$ openssl req -new -key ~/.ssl/PUBLIC.key -out ~/.ssl/REQUEST.csr

# 4. 根据CSR生成证书 Certificate
$ openssl x509 -req -days 365 -in ~/.ssl/REQUEST.csr -signkey ~/.ssl/PUBLIC.key -out ~/.ssl/CERTIFICATE.crt
```
