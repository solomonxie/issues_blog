...
$ openssl rsa -passin pass:x -in ~/.ssl/PASS.key -out ~/.ssl/PRIVATE.key
$ rm ~/.ssl/PASS.key    # 删除临时pass key

# 3. 根据公钥生成一个CSR证书请求 Request
$ openssl req -new -key ~/.ssl/PRIVATE.key -out ~/.ssl/REQUEST.csr

# 4. 根据CSR生成证书 Certificate
$ openssl x509 -req -days 365 -in ~/.ssl/REQUEST.csr -signkey ~/.ssl/PRIVATE.key -out ~/.ssl/CERTIFICATE.crt
$ rm ~/.ssl/REQUEST.csr   # 删除临时request请求
```
