...
## 「Blocking」怎么对抗阻塞？怎么应对高并发？

换个角度，为什么我们要这么费力对抗GIL？也许一开始根本就不需要用`Threads`多线程？

一开始用多线程，只是为了防止`Blocking`，也就是`I/O阻塞`。
上面我们的Socket服务器中有两个地方是遇到I/O阻塞的：
- `sock.accpet()`
- `client.recv(..)`
- `client.send(..)`

也就是说，每次server遇到上面这三句话，都会卡在那，等待结果直到超时，所以卡在这一个地方可能就很多秒。
所以说，我们主要就是解决这几个blocking语句。

**Blocking怎么解决？——没法解决！客户端不传来数据，服务器就只能等！**
但是，我们可以做到的是，一个客户卡住了，不影响其他客户啊。
所以这就是处理高并发的思路——让把每个Task任务在各个平行世界里同时运行，一条线卡住了也不影响其它线。
目前能做到这种并行处理任务的方法有：
- Multiprocessing 多进程
- Threads 多线程
- Generator 迭代生成器
...
`Multiprocessing`多进程的缺点是每条线的内存耗费成本太高，一个任务一个进程的话，每个任务几MB内存，光几百几千个任务就造成几GB级内存消耗，这种消耗级完全不是一般服务器能扛住的。
`Threads`多线程的缺点是`GIL锁的不公平性`所带来的每条线的快慢悬殊可能性很大，这样的效果不是我们希望的。
`Generator`生成器的代码要更复杂一点，本质上`Coroutine`也是应用Generator这一套逻辑，完全摆脱了多线程实现并发处理。但是Generator在实际测试性能后发现，它并没能避免GIL的难题：因为别的高计算请求，被CPU强制降速，甚至降到0。
`Coroutine`协程一样**不能避免**GIL锁的计算资源分配不公问题。
