# ❖ 一个案例了解Python多进程、多线程、协程、高并发处理：自己实现一个简单的Web Server [动图|长文] [DRAFT]
...
为了**对抗GIL**，很多人提出了很多方案，比如利用`Queue`／`Pool`／`Deque`等等。
具体代码写起来让新人费解。但是的确能做到处理高并发时，不被其它人的新请求影响到当前任务的处理速度。

换个角度，为什么我们要这么费力对抗GIL？也许一开始根本就不需要用Threading多线程？
