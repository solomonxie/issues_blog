...
我们可以在一个ORM类中定义一个relationship，这样一来，我们不用给Father表添加children数据，而是直接关联到Child这个子类。比如：
```py
class Father(..): 
...
class Child(..):
    name = Column(..)
```
然后，每次我们使用`father.children`的时候，就会自动返回与这个father相关联的所有children了。

但是，
这个单纯定义的`relationship('子类名')`只能让父类调用子对象。反过来，如果要问children他们的父亲是谁，就不行了。
...
后来，SQLAlchemy发现这种只在一边定义双向性backref的方法有点不太直观，所以又添加了另一个参数`back_populate`参数，而这个back_populate参数是单向性的：也就是说，你要确立双方向关系就必须在两边的类中都声明一遍，这样比较直观。
比如：
```py
class Father(..): 
    children = relationship('Child')
