...
SQLAlchemy的实践中，这种多对多的关系不直接在两个表中体现。两个表始终是各自独立的没有半点关系。所有的关系关联都是需要另外抽象出一个单独的表，专门用来存储Mapping映射的关系。
...
radio_users = Table( 'radio_users'
    Column( 'user_id', Integer, ForeignKey('user.user_id') ),
    Column( 'radio_id', Integer, ForeignKey('radio.radio_id') )
)
...
class Radio(Model):
    radio_id = Column(...)
    subscriptions = relationship( 'Radio', secondary=radio_users, backref='users' )
```
...
上面的多对多定义，是用了具有双向性的`backref`参数，也就是只要在一个类中定义即可获得双向引用。
实际上为了直观，我们也可以用`back_populate`在两个表中都定义：
定义两个独立的ORM对象：
```py
class User(Model):
    user_id = Column(...)
    subscriptions = relationship( 'Radio',  secondary=radio_users, back_populate='users' )

class Radio(Model):
    radio_id = Column(...)
    subscriptions = relationship( 'User',  secondary=radio_users, back_populate='radio' )
```