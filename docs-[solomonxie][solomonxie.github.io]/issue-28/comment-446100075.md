...
`反向引用`参数总结：
- `backref`参数：在父类中定义即可。
- `back_populates`参数：必须在父子类中都定义，且属性名称必须严格对称。
...
实际上为了直观，我们也可以用`back_populates`在两个表中都定义：
定义两个独立的ORM对象：
```py
class User(Model):
    user_id = Column(...)
    subscriptions = relationship( 'Radio',  secondary=radio_users, back_populates='users' )

class Radio(Model):
    radio_id = Column(...)
    subscriptions = relationship( 'User',  secondary=radio_users, back_populates='radio' )
```