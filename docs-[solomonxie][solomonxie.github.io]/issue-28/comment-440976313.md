...
- `Full Outer Join 全连接：相当于`A or B`，代表两个表的并集，即两表合并所有字段和数据为一个表，没有的字段以null填充。
- `Right Join` 右连接：使用left表里所有数据，而right表中只保留匹配数据，且没有的字段以null填充。
- `Left Join` 左连接：使用right表里所有数据，而left表中只保留匹配数据，且没有的字段以null填充。


![image](https://user-images.githubusercontent.com/14041622/48918608-a7bf4480-eec8-11e8-9e3f-5c808f338dac.png)

SQL连接两表，不光要指定连接方式，还要指定主键-外键的对应关系，使用`ON`关键字。
