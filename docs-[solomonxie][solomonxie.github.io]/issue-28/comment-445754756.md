...
>>> session.query(User).order_by( User.birth )

# 筛选结果 .filter( True/False 表达式 )
>>> session.query(User).filter( User.name != 'Jason' )
>>> session.query(User).filter( User.name.like('%ed%') )    # Fuzzy search
>>> session.query(User).filter( User.id in [1, 2, 3] )    # IN
>>> session.query(User).filter( ~ User.id in [4, 5, 6] )   # NOT IN
>>> session.query(User).filter( User.school == 'MIT', User.age < 24 )   # AND
>>> session.query(User).filter( _or(User.school == 'MIT', User.age < 24) )   # OR
```