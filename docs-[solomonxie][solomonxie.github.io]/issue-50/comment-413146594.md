# `Probabilities in Density Curves`
...
We could use any `Graphic Calculator` or online calculator, and input the `Mean`, `Standard Deviation`, `Lower bound` (as the Max value), and `Upper bound` (just input a very large number like 9999).
