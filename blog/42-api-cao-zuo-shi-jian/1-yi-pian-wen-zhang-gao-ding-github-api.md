...
## 分页 (Pagination)
Github有很好的分页功能。因为有很多页面结果动辄几百条，Github服务器也吃不消，所以必须有分页。
之前说了怎么在url参数中加`page=`和`per_page`来自定义第几页和每页多少条数据。但是光这个不够，跑程序的话我们需要知道一共有多少页。
根据官方说明，GitHub API的官方说明叫`Traversing with Pagination`(https://developer.github.com/v3/guides/traversing-with-pagination/)。
具体用法就是，在url请求中加入`q=addClass`这个参数。然后API在返回Response时候，就会在Header中加入这样的信息：
```
Link: <https://api.github.com/repositories/41742246/issues/20/comments?q=addClass&page=2>; rel="next", 
<https://api.github.com/repositories/41742246/issues/20/comments?q=addClass&page=6>; rel="last"
```
其中可以清楚的看到，下一页的url地址，以及最后一页的url地址，还可以看到最后一页的页码。
那么就是说，跑程序时候，只需要在url中加入`q=addClass`参数，然后读取Link返回值中的`rel="last"`中的页码，就可以知道一共多少页，然后创建一个for循环逐页读取了。
或者，每次直接读取`rel="next"`中的url，直到当前的url等于`rel="last"`中的url，这样比较合理。
