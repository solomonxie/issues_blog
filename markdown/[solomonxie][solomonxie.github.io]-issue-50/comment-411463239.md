...
> `Bivariate` is just a fancy way to say: for each x-point there's a corresponding y-point. etc., at point `(2,3)`, including x-point is 2 and y-point 3. For calculating `correlation coefficient` we need to treat x & y separately.
